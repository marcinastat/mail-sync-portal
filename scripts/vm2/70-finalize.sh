...
# clamav-milter świadomie WYŁĄCZONY (40-clamav.sh) — poczta wchodzi przez imapsync
# (IMAP APPEND), nie SMTP, więc milter nie ma czego skanować. Pokrycie AV daje
# clamav-maildir-scan.timer (skan maildirów). Dlatego NIE ma go tu na liście.
UNITS=(firewalld "postgresql-${PGDG_MAJOR_VERSION}" postfix dovecot clamd@scan clamav-freshclam clamav-maildir-scan.timer vm2-disk-check.timer vm2-provisioning-api)
FAILED=0
