#!/usr/bin/env bash
# Narzędzie ratunkowe (konsola VM2) — przywraca kopię configów zrobioną przez
# panel PRZED aktualizacją systemu. Uruchamiać jako root bezpośrednio z konsoli,
# gdy po aktualizacji coś się rozjechało. NIE jest wołane przez API.
#
#   vm2-config-recovery.sh list                 — pokaż dostępne kopie
#   vm2-config-recovery.sh show   <timestamp>   — pokaż zawartość kopii
#   vm2-config-recovery.sh restore <timestamp>  — przywróć kopię (z potwierdzeniem)
set -uo pipefail

BACKUP_ROOT="/var/lib/vm2-config-backups"
HEALTH_UNITS=(postgresql-17 dovecot postfix clamd@scan vm2-provisioning-api)
