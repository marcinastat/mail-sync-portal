...
from ..schemas import ScanScheduleRequest, SystemRebootRequest, SystemUpdateRequest, SystemUpdateResult
...
def scan_schedule(
    ...
    result = system_control.apply_scan_schedule(body.model_dump())
    insert_audit_log(
        conn, actor=actor, action="scan.schedule", target_type=None, target_id=None,
        details=body.model_dump(), source_ip=request.client.host if request.client else None,
    )
    return result
