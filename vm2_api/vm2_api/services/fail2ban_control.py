"""Status fail2ban na VM2 (read-only) — do kafelka w panelu VM1. Uruchamiane
przez konto vm2-api przez sudo-helper (fail2ban-client wymaga roota). Zwraca
sparsowany status; NIC nie modyfikuje (unban/ban tylko z konsoli)."""

import re
import subprocess

HELPER = "/usr/local/sbin/vm2-fail2ban-status.sh"
...
def get_status() -> dict:
    try:
        out = subprocess.run(["/usr/bin/sudo", "-n", HELPER], capture_output=True, text=True, timeout=10)
